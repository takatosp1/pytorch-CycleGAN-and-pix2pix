...
class SemiPix2PixModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netG = networks.define_gated_G(opt.input_nc, opt.output_nc, opt.ngf,
                                      opt.which_model_netG, opt.norm, not opt.no_dropout, opt.init_type, self.gpu_ids, opt.use_gt_mask)
    ...
    def forward(self):
        ...
        if self.opt.gate_on_gt:
            self.real_B_to_comp = self.gated_B
        else:
            self.real_B_to_comp = self.real_B
    ...
    def backward_G(self):
        ...
        for stream_num in range(self.opt.num_stream):
            ...
            # TODO: should detach() real_B_to_comp if it's generated by real_B *generated_mask
            self.loss_G_L1 = self.criterionL1(
                self.fake_B[stream_num], self.real_B_to_comp[stream_num].detach()) * self.opt.lambda_L1
